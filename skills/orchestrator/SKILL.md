...
3. When delegation is explicitly authorized, this root orchestrator session delegates independent repositories to separate OpenClaw subagent worker sessions (`sessions_spawn` with a stable `taskName`). Whenever assigning or materially changing work, rename the worker session to `<Project>: <short current task>`. Keep work for one repository in its existing worker session. Do not request a custom model unless the owner specifies one; inherit the platform default.
4. Keep this coordinator session lightweight. Do not perform extensive repository work here. Delegate it to a repository worker, then monitor by reading current state via `subagents(action=list)` and `subagents(action=transcript)`.
...
- Only this root orchestrator session may create, reuse, fork, assign, rename, archive, or steer worker sessions.
- Repository workers perform only their assigned repository work and report results to this orchestrator. They must not create subworkers, delegate work, or manage other workers.
- Put the no-subdelegation rule in every worker prompt.
- Do not delegate portfolio triage, worker creation, or worker management to another worker.
...
1. Read the worker's latest current state via `subagents(action=transcript, taskId=<id>)`, including its newest user/delegation messages and active turn.
2. Treat the newest worker-local instruction as authoritative over older orchestration plans.
...
Never interrupt, archive, rename, duplicate, or replace a worker without first reading its current state. For a suspected duplicate, read both workers; if either has unique progress, edits, or an active turn, leave it alone and ask the owner before changing worker state.

## Worker Naming

- Rename a worker whenever giving it a new task or materially changing its assignment. With OpenClaw subagents, rename by adjusting the `taskName` on the next spawn or message.
- Format every worker title as `<Project>: <short current task>`.
- Read the latest state and newest worker-local instructions before renaming.
...
An idle or completed repository worker must not remain a polling-only lane. After reading its latest state, inspect that repository's current queue, CI, latest release, package metadata, and unreleased changelog. Then do exactly one:

1. Assign the next autonomous issue or PR to the same repository worker.
...
Do not keep completed workers merely to satisfy a lane count. A monitored repository should have active autonomous work, a pending owner question, an active release, or a documented reason no release is warranted.
...
Assume maintainer credentials are stored in a managed secret store (1Password, macOS Keychain, environment, etc.). Before reporting a credential blocker:

1. Check only the exact expected environment variable; use it only when already exported.
2. Read the service-specific auth skill or doc, then use the project's documented secret-fetch path.
3. Prefer the scoped service-account path; use the required persistent shell session and exact known item/vault/field.
4. Never broadly enumerate secrets or print values. Use `op run` / `op inject` (or the equivalent for the configured secret store) when supported.
5. Ask the owner only after the targeted secret-store path is absent, inaccessible, or requires interactive unlock/approval.

Keep credential discovery and use inside the worker that needs the secret. Report only presence, access path, and the exact missing approval or item; never send credentials between workers.

## Worker Contract

Every delegated implementation worker, within its explicit authorization, must:
...
- run any project-mandated review step (project linter, `autoreview`, or a configured `code-review` skill) until no accepted/actionable findings remain;
...
- after authorized landing, return to updated, clean default branch.
...
Use the repository's release docs and matching toolchain:

- npm packages: use `npm`;
- macOS apps: use the project's release script or workflow;
...
Then open the next patch `Unreleased` section. Commit and push the closeout only when those mutations are authorized; otherwise leave the verified local closeout ready and report the exact permission needed. After an authorized push, pull `--ff-only` and finish on the clean default branch.
...
Report meaningful changes, not routine polling. Maintain a heartbeat automation (cron or OpenClaw scheduler) when the user asks to keep monitoring.
